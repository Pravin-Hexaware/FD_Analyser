...
async def extract_annual(report: ExtractAnnualRequest) -> List[dict]:
    """
    Extract Annual (or Half-yearly) statements from multiple XBRL/iXBRL URLs.
    Uses the same calculation method as quarterly extraction - returns flat metrics dict.
    """
    if not report.url:
    ...
    for url in report.url:
        try:
            if url.endswith(".xml"):
                ...
                # Verify we have a dict before converting
                if not isinstance(extracted_data_grouped, dict):
                    raise Exception(f"extract_xbrl_data returned {type(extracted_data_grouped).__name__} instead of dict")
                ...
                response.append({
                    "url": url,
                    "type": "unknown",
                    "error": "Unsupported file type. Provide .xml or iXBRL (.html/.htm/.xhtml)"
                })
                continue

            # Verify extracted_data is a list
